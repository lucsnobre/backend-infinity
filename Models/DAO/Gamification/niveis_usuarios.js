...
const adicionarExperiencia = async function (id_usuario, pontos_xp) {
    try {
        let sql = `
            UPDATE niveis_usuarios 
            SET pontos_experiencia = pontos_experiencia + ?, atualizado_em = ? 
            WHERE id_usuario = ?
        `
...
const atualizarTempoReproducao = async function (id_usuario, minutos_adicionais) {
    try {
        let sql = `
            UPDATE niveis_usuarios 
            SET tempo_total_reproducao = tempo_total_reproducao + ?, atualizado_em = ? 
            WHERE id_usuario = ?
        `
...
const incrementarMusicasDescobertas = async function (id_usuario) {
    try {
        let sql = `
            UPDATE niveis_usuarios 
            SET total_musicas_descobertas = total_musicas_descobertas + 1, atualizado_em = ? 
            WHERE id_usuario = ?
        `
...
const verificarEAtualizarNivel = async function (id_usuario) {
    try {
        ...
        if (novoNivel > nivelAtual.nivel) {
            let sqlUpdate = `
                UPDATE niveis_usuarios 
                SET nivel = ?, atualizado_em = ? 
                WHERE id_usuario = ?
            `
            await prisma.$queryRawUnsafe(sqlUpdate, novoNivel, new Date(), id_usuario)
...
const obterProgressoNivel = function (xp_atual, nivel_atual) {
    ...
    const xpProximoNivel = calcularXPTotalNivel(nivel_atual)
    ...
    return {
        ...
        xp_total_proximo_nivel: xpProximoNivel,
    ...
    try {
        let sql = `
            SELECT 
                ...
                AVG(total_musicas_descobertas) as media_musicas,
                SUM(total_musicas_descobertas) as total_musicas
    ...
    try {
        let sql = `
            SELECT 
                ...
                AVG(tempo_total_reproducao) as media_tempo_nivel
            FROM niveis_usuarios
            GROUP BY nivel
            ORDER BY nivel ASC
        `
...
const obterPosicaoRanking = async function (id_usuario) {
    try {
        let sql = `
            SELECT 
                (SELECT COUNT(*) + 1 FROM niveis_usuarios nu2 
                 WHERE (nu2.nivel > nu.nivel) OR (nu2.nivel = nu.nivel AND nu2.pontos_experiencia > nu.pontos_experiencia)
                ) as posicao_ranking_nivel,
                (SELECT COUNT(*) + 1 FROM niveis_usuarios nu3 
                 WHERE nu3.pontos_experiencia > nu.pontos_experiencia
                ) as posicao_ranking_xp,
                (SELECT COUNT(*) + 1 FROM niveis_usuarios nu4 
                 WHERE nu4.tempo_total_reproducao > nu.tempo_total_reproducao
                ) as posicao_ranking_tempo,
                (SELECT COUNT(*) + 1 FROM niveis_usuarios nu5 
                 WHERE nu5.total_musicas_descobertas > nu.total_musicas_descobertas
                ) as posicao_ranking_musicas
            FROM niveis_usuarios nu
            WHERE nu.id_usuario = ?
        `
        let result = await prisma.$queryRawUnsafe(sql, id_usuario)
